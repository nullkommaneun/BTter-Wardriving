export function applyFilters(rows, f){
  ...
  if(f.fastpair){ out = out.filter(r => (Array.isArray(r.serviceUUIDs) && r.serviceUUIDs.some(u => (u||'').toLowerCase().includes('fef3'))) || (r.category||'').toLowerCase().includes('fast pair')); }
  if(f.industrie){ 
    const hasFcf1 = (r) => { const svc = r.serviceData || {}; const keys = Object.keys(svc).map(k=>k.toLowerCase()); return keys.includes('0000fcf1-0000-1000-8000-00805f9b34fb') || keys.includes('fcf1'); };
